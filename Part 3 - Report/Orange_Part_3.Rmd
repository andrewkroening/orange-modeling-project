...
Even though using only the model given by backward stepwise selection gives us a model with higher adjusted $R^2$ and a lower BIC value, we will choose the model that combines this result with our *a priori* variables to study their relationship with the response variable. To check if our final model follows the linear regression assumptions, we will plot the model summary plots. The resulting plots are available in Appendix D.
...
We observe from our Adjusted R-squared value 0.82, meaning that 82% of the variance in life expectancy is explained by the model. 

According to the statistics, for the residual summary statistics, we can see that the median should be close to 0, i.e., the mean of the residuals. 3Q and 1Q should be quantitatively close to each other and symmetrically distributed, and the errors follow a Gaussian distribution. Based on the p-value we get from the variables, we can conclude that the above five essential factors have an impact on life expectancy. If all other variables are held constant, Adults death probability increase by 0.1% will decrease Life expectancy by 0.03 years (11 days). General government expenditure on health Rise by 1% will increase Life expectancy by 0.03 years (11 days).HIV.aids death rates (which are kids from 0-4 yrs) Rise of 0.1% will decrease Life expectancy by 1.04 years (380 days). Average BMI Increase of one unit can increase Life expectancy by 0.04 years ( 17 days). Year of Schooling Rise per year will increase Life expectancy by 1.18 years ( 431 days). We also observe from our Adjusted R-squared value 0.82, meaning that the model explains 82% of the variance in life expectancy.
...
We also examine multicollinearity concerns for all four model fits. The Variance Inflation Factors table in Appendix F clearly shows that the first model has significant problems, with two variables scoring a VIF of over 10. After we remove one variable, all subsequent models are satisfactory, with no variables scoring high on this test.
...
\newpage
**Appendix F - VIF Table, Question 2**
