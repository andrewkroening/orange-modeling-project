...
df_2014_cleaned <- df_pop[,c("Country", "2014")]
df_2014 <- merge(df_life_expectancy_2014, df_2014_cleaned, by = "Country")
...
The dataset for our analysis has 22 variables, as previously described. For a complete description of all variables, see Appendix A. The categorical and outcome variable for our second question is country development status. We check the relationship between this variable and the outcome variable for our first question, life expectancy. We use boxplots to find that the life expectancy in developed countries is higher, which means the categorical variable might be a good predictor for the model in the first question. Another interesting observation useful in understanding the factors that affect a country's development status is the boxplot of GDP vs. Status. We see that the median GDP is higher for developed countries. We also see from the Schooling vs. Status plot that the median years of schooling is larger for developed countries than for developing countries. The full summary of EDA plots is available in Appendix B.

We selected the *a priori* variables we had set earlier for the continuous variables and examined those relationships. First, we examined the schooling, income, thinness, GDP, and HIV variables and found linear relationships consistent with the initial prediction of the variables of interest. After we dropped correlated pairs showing significantly high VIFs, we were left with the resulting dataset for analysis. Descriptive information about the variables used to address question 1 can be found in Table 1 (below).
...
We then proceeded to model fitting. We use a linear regression model with life expectancy as our response variable to answer the first research question. We fit three models to study the relationship between the independent variables and life expectancy, including only our *a priori* variables, using backward stepwise selection, and our final model, which combines our *a priori* variables with the result of the backward stepwise selection (Appendix C). During this process, we found that the Income composition of resources variable was, in fact, analogous to Human Development Index. We chose not to include it in our model for two reasons:

* Human development index is a measure that already factors life expectancy, our response variable.

* We suspected that it had a high correlation with a country's development status. 

To test this, we fit a logistic regression model with development status and income composition of resources. After we found that there was indeed a correlation, we excluded the income composition of resources from our list of *a priori* variables. Instead, we included the development status of a country and the average years of schooling of the population. 

We evaluated the models using adjusted $R^2$ and BIC to find the best-fit model. Finally, we obtain diagnostic information about the performance of our models.

```{r model1_backward, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
...
The model given by backward stepwise selection provides us with a model with a higher adjusted $R^2$ and a lower BIC value; however, we will choose the model that combines this result with our *a priori* variables to study their relationship with the response variable. We will plot the model summary plots to check if our final model follows the linear regression assumptions. The resulting plots are available in Appendix D.
...
We observe from our Adjusted R-squared value 0.82, meaning that the model explains 82% of the variance in life expectancy. 

According to the statistics, for the residual summary statistics, we can see that the median should be close to 0, i.e., the mean of the residuals. 3Q and 1Q should be quantitatively close to each other and symmetrically distributed, and the errors follow a Gaussian distribution. Based on the p-value we get from the variables, we can conclude that the above five essential factors impact life expectancy. If all other variables are held constant:

* An increase of 0.1% in adult death probability will decrease life expectancy by 0.03 years (11 days). 
* A 1% increase in government expenditure on healthcare will increase Life expectancy by 0.03 years (11 days)
* A rise in the HIV/AIDS death rate (ages 0-4 yrs) will decrease Life expectancy by 1.04 years (380 days). 
* An increase in average BMI by one unit can increase life expectancy by 0.04 years ( 17 days). 
* Each additional year of schooling will increase life expectancy by 1.18 years ( 431 days).

## Question 2: (Inference)

### Results

From the Logistic Regression Models output table in Appendix E, we observe that only one predictor variable has a p-value less than 0.05 denoting it as a significant predictor: *Years of Schooling*. The interpretation of this predictor's coefficient in terms of the odds of a country being identified or labeled as a developed country is: while holding all other predictor variables constant, a one-year increase in *Years of Schooling* make it 2.39 ($e^{1.05}$) times more likely that country being identified as a developed country.

We also examine multicollinearity concerns for all four model fits. The Variance Inflation Factors table below clearly shows that the first model has significant problems, with two variables scoring a VIF of over 10. After we remove one variable, all subsequent models are satisfactory, with no variables scoring high on this test.
...
Before assessing the accuracy of the potential model fit (3rd model) for this research question, we realize the interpretation of the *Income Composition of Resources* variable might be troublesome. It was recorded as Human Development Index in terms of income composition of resources (index ranging from 0 to 1); however, its meaning could be more transparent. We decided to adjust the model to omit the *Income Composition of Resources* variable and try to predict the development status of each country in the dataset. Those predictions are used to build a confusion matrix, shown in the table below. From this table, we can determine the True Positive, True Negative, False Negative, and False Positive rates by comparing predicted and actual values. For prediction, we first use the threshold of 0.5 to classify countries as developed or developing. The accuracy of this model is approximately 91\%, with a 95% Confidence Interval of within 83% ~ 95%.
...
le14_cont_clean_subset <- df_2014_cleaned[,c("Life.expectancy", "percentage.expenditure","Measles","BMI","Polio","Total.expenditure" ,"HIV.AIDS","GDP","pop","Income.composition.of.resources", "Schooling")]
...
**Appendix C - Linear Regression Models, Question 1**
...
models <- list(mlrmod, m, final_model)
mod.names <- c('apriori', 'backwardstepwise', 'final_model')
...
stargazer(mlrmod,m,final_model,
          ...
          covariate.labels=c("Status Developing countries","Population", "Years of Schooling", "Measles recorded per 1000 people", "Polio Immunization Coverage", "Adult Mortality num per 1000 people", "Infant Deaths per 1000 people", "Under Age 5 Deaths per 1000 people","HIV/AIDS Deaths/1000 live births", "GDP per capita", "BMI","Std. Income Composition of Resources","Gov. Expenditure on Healthcare"),
          type='latex')
```

\newpage
**Appendix D - Residual Plots, Question 1**
...
<!-- **Appendix E - BIC Table, Question 1** -->

<!-- ```{r bic_table, results="asis", echo=FALSE, header=FALSE, message=FALSE, warning=FALSE, cache=FALSE} -->
<!-- stargazer(bic_tab, -->
<!--           header=FALSE, -->
<!--           title = "BIC Table", -->
<!--           font.size = "small", -->
...
\newpage
**Appendix E - Logistic Regression Models, Question 2**

Table below shows the output of 8 predictor variables regressed onto country development status in 4 models. 

```{r logistic_output, results="asis", out.width="90%", echo=FALSE, header=FALSE, message=FALSE, warning=FALSE}
...
From left to right, those models are: 
