...
```{r histograms, echo=FALSE, results='asis'}
...
for(i in 1:ncol(le14_cont_clean_subset)){
  ...
  ggp <- ggplot(le14_cont_clean_subset, aes_string(x = col)) +
    ...
    geom_vline(xintercept = mean(le14_cont_clean_subset[[col]]), col = "red", lwd=1.5) 
...
```{r plot missing data 1,fig.width=16,fig.height=4,echo=FALSE,fig.show='hold'}
missing.rows = dim(df_life_expectancy_2014)[1] -  dim(na.omit(df_life_expectancy_2014))[1]
missings_df <- df_life_expectancy_2014.frame(type=c("missing", "non-missing") ,count = c(missing.rows,  dim(na.omit(df_life_expectancy_2014))[1]))
...
```
```{r visualize missing data 2,fig.width=16,fig.height=8,echo=FALSE,fig.show='hold'}
missing_counts <- df_life_expectancy_2014.frame(feature = factor(names(df_life_expectancy_2014)),
                    counts=sapply(df_life_expectancy_2014, function(x) sum(is.na(x))))
