...
df_life_expectancy <- read.csv("Life Expectancy Data.csv")
...
ggplot(data = df_life_expectancy_2014, aes(x = Life.expectancy, y = Status)) +
  ...
  labs(title = 'Boxplots of Life Expectancy by Status',
       y = "Status",
       x = "Life Expectancy") +
...
There is a second challenge posed in some of the variables in the dataset by a skewed distribution. When checking boxplots of all variables, we found 4 variables may need transformation before inclusion in the final model.
...
We may want to use log transformation for population and GDP, because of the magnitude of gaps. But for other two, we need more investigation due to the difficulty of interpretation. The boxplots and histograms of the population and GDP variables after log transformation yield a better distribution:
...
\newpage
#### "Figure XXX - Histgrams for Potential Predictor Variables" 
\
\
